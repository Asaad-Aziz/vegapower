...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Tamara verify-app called:', { orderReferenceId, email, plan, amount, authMethod })

    if (!orderReferenceId || !email) {
    ...
    const tamaraPaymentId = `tamara_app_${orderReferenceId}`

    const { data: existing } = await supabase
      .from('app_subscriptions')
      .select('id')
      .eq('payment_id', tamaraPaymentId)
      .single()
    ...
    const paidAmount = parseFloat(amount) || 187
    ...
    const { error: orderError } = await supabase.from('orders').insert({
      ...
      status: 'paid',
      moyasar_payment_id: tamaraPaymentId,
      discount_code: discountCode || null,
    })
    ...
    console.error('Tamara verify-app error:', error)
    return NextResponse.json(
